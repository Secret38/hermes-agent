...
[replayEvents])
  ...
  useEffect(() => {
    setCursor(Math.max(eventCount - 1,
[taskId,
[taskId,
{
    hour: '2-digit',
...
{ id: 'safety',
{ id: 'verification',
0.78fr)_minmax(24rem,
0),
0))
    setSelectedSequence(lastSequence)
  },
0))
  const replayEvents = allEvents.length ? allEvents.slice(0,
0)}
                min={0}
                onChange={event => {
            ...
            <div className="grid min-h-[30rem] gap-3 xl:grid-cols-[minmax(20rem,
1.22fr)]">
...
1fr)] items-start gap-2 border-b border-(--ui-stroke-tertiary) px-2.5 py-2 text-left transition-colors',
2)}
        </pre>
      </div>
    </div>
...
function compactId(value: null | string | undefined): string {
  ...
  return value.length <= 20 ? value : `${value.slice(0,
...
event,
eventCount,
fetchAgentOSSnapshot } from './api'
import type { AgentOSEvent,
filter === item.id
          ...
          <div className="mx-auto grid w-full max-w-[1600px] gap-3">
            <div className="rounded-lg border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) p-3">
              ...
              <input
                ...
                disabled={!eventCount}
                max={Math.max(eventCount - 1,
filter: EventFilter): boolean {
  if (filter === 'all') {
    return true
  }
...
filter)),
filter])
  const safeCursor = Math.min(Math.max(cursor,
...
function eventMatches(event: AgentOSEvent,
label: 'Checkpoints' },
label: 'Recovery' },
        ...
        <div className="flex min-w-0 items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-3xl text-xs leading-relaxed text-(--ui-text-tertiary)">
              Replay the append-only execution ledger,
...
onSelect
  ...
  return (
    <button
      className={cn(
        'grid w-full grid-cols-[4.5rem_1.5rem_minmax(0,
onSelect
}: {
  selected: string
  tasks: AgentOSTask[]
...
refetchInterval: 5_000
  ...
  const [taskId,
safeCursor + 1) : []
  const state = useMemo(() => replayState(replayEvents),
second: '2-digit'
  }).format(date)
}
...
setCursor] = useState(0)
  const [selectedSequence,
setFilter] = useState<EventFilter>('all')
  const [cursor,
setSelectedSequence] = useState<number>()
...
tasks])
  ...
  const filteredEvents = useMemo(() => allEvents.filter(event => eventMatches(event,
useMemo,
