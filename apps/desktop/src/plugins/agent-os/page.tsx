...
'controlled apps may have their own egress'],
'EXECUTING',
'FAILED'].includes(action.state))
      .map(action => ({
        id: `action:${action.id}`,
'FAILED'].includes(agent.state))
      .map(agent => ({
        id: `agent:${agent.id}`,
'FAILED'].includes(task.state)) {
    return null
  }
...
'RUNNING',
'STARTING',
'starting'].includes(session.status)).length
...
'WAITING_FOR_USER',
'waiting',
[...(childrenByParent.get(agent.parent_agent_id) ?? []),
[...(deps.get(edge.step_id) ?? []),
[...(depsByStep.get(edge.step_id) ?? []),
[...(grouped.get(level) ?? []),
['Browser',
['Computer use',
['MCP',
...
#d49b45_7%,
#d49b45_28%,
`${network.data.mcp.enabled}/${network.data.mcp.configured} enabled`],
0.62fr) minmax(24rem,
0.65fr)]">
  ...
  const nodes = [
    {
      id: 'workspaces',
0)
  const active = sessions.filter(session => ['working',
1.5fr) minmax(16rem,
1.35fr)_minmax(18rem,
1fr)_auto] gap-x-3 gap-y-1 rounded border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) px-2.5 py-2"
                  key={event.id}
                >
                  <div className="min-w-0">
    ...
    <div className="aos-panel grid min-h-72 place-items-center p-8 text-center">
      <div className="max-w-lg">
        ...
        {snapshot.store_error && (
          <div className="mt-4 rounded-md border border-[color-mix(in_srgb,
3)
                          .join(' · ')}
                      </div>
                    )}
...
8)
  ...
  if (!items.length && !['WAITING_FOR_APPROVAL',
8)
                    .map(([key,
9)}…${value.slice(-6)}`
}
      ...
      <div className="space-y-1.5">
        {ordered.map(action => (
          <div className="rounded-md border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) px-2.5 py-2" key={action.id}>
            ...
            {action.error && (
              <div className="mt-2 rounded bg-[color-mix(in_srgb,
18).map(event => (
      ...
      <section className="aos-panel overflow-hidden">
        ...
        <div className="aos-scrollbar max-h-[22rem] overflow-y-auto p-2.5">
          {memory.relations.length === 0 ? (
            <div className="p-3 text-xs text-(--ui-text-tertiary)">Relations appear as tasks bind to workspaces,
...
AgentOSAgent[]>()
  ...
  for (const agent of agents) {
    ...
    childrenByParent.set(agent.parent_agent_id,
AgentOSEvent,
AgentOSPlan,
AgentOSPlanStep,
AgentOSPlanStep[]>()
...
b) =>
    ...
    .slice(0,
b) => b.priority - a.priority || a.created_at.localeCompare(b.created_at)))
}

function PlanGraph({ plan }: { plan: AgentOSPlan | null | undefined }) {
  const [selectedStepId,
be audited and be explained after a restart.
        </p>
      </div>
...
function OperationsView() {
  ...
  const rows = operations.snapshots.flatMap(snapshot =>
    snapshot.tasks.map(task => ({ snapshot,
...
Codicon,
command text,
commands and prompt content are intentionally absent.
        </p>
      </div>
...
function SectionHeader({
  icon,
compact = false }: { state: string; compact?: boolean }) {
  return (
    <span
      className={cn(
...
depth
  ...
  agent: AgentOSAgent
  childrenByParent: Map<string,
depth > 0 && 'ml-4 border-l border-(--ui-stroke-tertiary) pl-3')}>
      {depth > 0 && (
...
function AgentsPanel({ agents }: { agents: AgentOSAgent[] }) {
  ...
  const childrenByParent = new Map<string,
detail:
...
detail: 'bounded retries / replans' },
detail: 'Conversation lineage into tasks'
    },
detail: 'L2+ actions observed' },
detail: 'ledger evidence events' },
...
function AttentionQueue({ task }: { task: AgentOSTask }) {
  const items = [
    ...task.actions
      .filter(action => ['WAITING_PERMISSION',
  ...
  return (
    <section className="aos-panel overflow-hidden">
      ...
      <div className="aos-topology p-5">
        <div className="aos-topology-core rounded-lg border border-[color-mix(in_srgb,
    ...
    <section className="aos-panel overflow-hidden">
      ...
      <div className="aos-process-map aos-scrollbar overflow-x-auto p-3">
        <div className="flex min-w-max items-stretch">
          {stages.map((stage,
detail: waiting ? `${waiting} waiting` : 'no pending gate' },
detail]) => (
    ...
    <div className="p-3.5">
      ...
      <div className="aos-plan-canvas aos-scrollbar">
        <div className="aos-plan-layers">
          {layers.map((layer,
error,
exactWorkerRoute } from './selectors'
import { ExecutionCanvas,
fallback)
  }
...
import { ExternalConnectionsSection,
filter === item.id
                ? 'border-[color-mix(in_srgb,
headers,
headers or tool output are stored.
        </div>
      </section>
    </div>
  )
...
function FleetView() {
  ...
  const subagents = sessions.reduce((total,
host,
icon: 'checklist' },
...
icon: 'shield' }
  ...
  return (
    <section className="agent-os-page flex min-h-0 flex-col">
      <header className="aos-mission-header shrink-0 border-b border-(--ui-stroke-tertiary) bg-[color-mix(in_srgb,
icon: 'target',
icon: 'tools',
icon: 'type-hierarchy' },
icon: 'verified',
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-xs leading-relaxed text-(--ui-text-tertiary)">
              The graph below is runtime truth,
index) => (
      ...
      {recovery && (
        <div className="flex items-center gap-2 border-t border-(--ui-stroke-tertiary) bg-[color-mix(in_srgb,
isFetching,
its steps and dependencies appear here from the Agent OS ledger.
          </div>
        </div>
      </div>
    )
  ...
  const titleById = new Map(plan.steps.map(step => [step.id,
label,
label: 'Actions' },
label: 'Agents',
label: 'Agents' },
label: 'All execution events' },
label: 'Approvals',
label: 'Checkpoints',
label: 'Checkpoints',
label: 'Connections',
...
meta,
minute: '2-digit',
network.data.browser.class,
network.data.computer_use.class,
network.data.mcp.enabled ? 'mixed' : 'disabled',
network.data.messaging.class,
network.data.model_provider.class,
network.data.model_provider.provider],
network.data.telemetry.class,
network.data.telemetry.transmission_enabled ? 'transmitting' : 'not transmitting'],
network.data.updates.class,
network.data.updates.mode]
      ]
    : []
    ...
    <div className="space-y-3">
      <div className="aos-panel p-4">
        ...
        <p className="mt-2 max-w-4xl text-xs leading-relaxed text-(--ui-text-tertiary)">
          ...
          it does not mean offline. Raw endpoints,
not a configured wish-list. Health probes represent the durable store,
number>()
...
routes)
                ...
                aria-pressed={tab === item.id}
                          className="rounded border border-(--ui-stroke-tertiary) px-2 py-1 text-[0.58rem] text-(--ui-text-secondary) hover:bg-(--ui-control-hover-background)"
                          onClick={() => openHermesSession(task.originSessionId!,
routes)
                const originRoute = exactOperationsRoute(snapshot.connectionId,
RuntimeObservatory,
RuntimeTopologyCanvas } from './visual-intelligence'
import type {
  AgentOSAction,
second: '2-digit'
  }).format(date)
...
selected,
selectedId,
SemanticMemorySection } from './context'
import { MissionControlActions } from './control'
import { useHermesOperations,
session) => total + session.subagents.length,
sessions and checkpoints.</div>
          ) : (
            <div className="grid gap-1 md:grid-cols-2">
              {[...memory.relations].slice(-60).reverse().map((edge,
setFilter] = useState<TimelineFilter>('all')
...
setQuery] = useState('')
  ...
  const { data: snapshot,
setSelectedActionId] = useState<string>()
...
setSelectedId] = useState<string>()
  const [query,
setSelectedStepId] = useState<string>()
      ...
      <div className="grid min-h-56 place-items-center px-8 text-center">
        <div>
          ...
          <div className="mt-1 max-w-md text-xs leading-relaxed text-(--ui-text-tertiary)">
            Once the planner compiles a goal,
setTab] = useState<MissionTab>('overview')
  const [selectedId,
snapshot,
snapshot
}: {
  ...
  return (
    <div className="grid min-h-0 gap-3 xl:grid-cols-[19rem_minmax(0,
snapshot
}: {
  onSelect: (id: string) => void
  query: string
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        style={{ gridTemplateColumns: 'minmax(14rem,
snapshot.profile,
snapshot])
...
states,
step.title]))
  const depsByStep = new Map<string,
step])
  }
          ...
          {(tab === 'overview' || tab === 'tasks') && (
            <div className="relative w-64 max-w-full">
              ...
              <input
                ...
                className="h-7 w-full rounded-md border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) pl-7 pr-2 text-[0.68rem] text-foreground outline-none placeholder:text-(--ui-text-quaternary) focus:border-[color-mix(in_srgb,
task }) => {
                const source = operations.sources.find(item => item.id === snapshot.sourceId)
                const workerRoute = exactWorkerRoute(task,
task }))
  )
...
transparent)] p-2 text-[0.6rem] leading-relaxed text-destructive">
...
function SafetyStrip({ task }: { task: AgentOSTask }) {
  ...
  const items = [
    { icon: 'shield',
transparent)] p-2 text-left text-[0.65rem] text-destructive">
            {snapshot.store_error}
          </div>
        )}
      </div>
...
export function AgentOSMissionControl() {
  const [tab,
transparent)] px-2 py-1.5 text-[0.62rem] leading-relaxed text-destructive">
                {action.error}
              </div>
            )}
          </div>
            ...
            <div className="mt-3 rounded border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) p-2">
              ...
              <div className="mt-2 grid gap-1">
                {Object.entries(selectedAction.verification_result ?? {})
                  .slice(0,
transparent)] px-3 py-2 text-[0.6rem] text-(--ui-text-tertiary)">
...
function Overview({
  onSelect,
transparent)] px-4 pb-3 pt-4 backdrop-blur">
...
URLs,
useLiveFleet } from './operations-data'
import { ProjectsView } from './projects'
import { useComputerUseSecurity,
useNetworkSecurity,
...
useTelemetrySecurity } from './security-data'
import { openHermesSession } from './session-navigation'
import { exactOperationsRoute,
value
}: {
  icon: string
  label: string
...
function planLayers(plan: AgentOSPlan): AgentOSPlanStep[][] {
  const byId = new Map(plan.steps.map(step => [step.id,
value: highRisk,
value: memory.checkpoints.length,
value: memory.sessions.length,
value: memory.workspaces.length,
value: snapshot.summary.events,
value: task.metrics.approvals,
value: task.metrics.checkpoints,
value: task.metrics.recoveries,
value: task.metrics.verifications,
value]) => (
...
function eventGroup(type: string): Exclude<TimelineFilter,
value]) => (
                    <div className="flex min-w-0 justify-between gap-3 text-[0.58rem]" key={key}>
          ...
          {selectedAction.error && (
            <div className="mt-2 rounded border border-[color-mix(in_srgb,
value]) => (
...
var(--dt-destructive)_7%,
var(--dt-destructive)_8%,
var(--dt-destructive)_30%,
var(--dt-destructive)_35%,
var(--dt-primary)_7%,
var(--dt-primary)_9%,
var(--dt-primary)_35%,
var(--dt-primary)_40%,
...
var(--dt-primary)_45%,
var(--dt-primary)_45%,
var(--ui-bg-primary)_92%,
var(--ui-bg-secondary))] p-2.5"
              key={item.id}
            >
              <div className="flex min-w-0 items-start justify-between gap-2">
                <div className="min-w-0">
...
function TaskDetail({ task }: { task: AgentOSTask }) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid min-h-0 gap-3 xl:grid-cols-[minmax(0,
var(--ui-bg-secondary))] p-3 text-center">
  ...
  return (
    <div className="space-y-3">
      <div className="aos-panel p-4">
        ...
        <p className="mt-2 max-w-3xl text-xs leading-relaxed text-(--ui-text-tertiary)">
          This graph is derived from the durable execution ledger: workspaces,
var(--ui-bg-secondary))] text-foreground'
        ...
        {rows.map(event => (
          <div className="aos-timeline-row" key={event.id}>
            <button
              ...
              className="aos-timeline-glyph hover:border-[color-mix(in_srgb,
var(--ui-stroke-tertiary))] bg-[color-mix(in_srgb,
var(--ui-stroke-tertiary))] bg-[color-mix(in_srgb,
var(--ui-stroke-tertiary))] bg-[color-mix(in_srgb,
var(--ui-stroke-tertiary))] bg-[color-mix(in_srgb,
var(--ui-stroke-tertiary))] bg-[color-mix(in_srgb,
var(--ui-stroke-tertiary))] hover:text-foreground"
        ...
        <div className="mt-3 rounded-md border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary) p-2.5">
          ...
          <div className="mt-3 rounded border border-(--ui-stroke-tertiary) bg-(--ui-bg-primary) p-2">
            ...
            {Object.entries(selectedEvent.payload ?? {}).length ? (
              <div className="mt-2 grid gap-1">
                {Object.entries(selectedEvent.payload ?? {}).map(([key,
var(--ui-stroke-tertiary))] px-2 py-1 text-[0.58rem] font-medium text-foreground hover:bg-(--ui-control-hover-background)"
          ...
          <div className="aos-scrollbar max-h-[24rem] overflow-y-auto p-2.5">
            <div className="space-y-1">
              {audit.data.events.map(event => (
                <div
                  className="grid min-w-0 grid-cols-[minmax(0,
workspaces…"
                value={query}
              />
            </div>
